...
class Level{
  ...
  Level(int numberMaxPoints, Kirlia k, Enemy enemy, 
    ArrayList<Point> coins, ArrayList<Rectangle> rectangle, PImage img){
  
    setupL(numberMaxPoints, k, enemy, coins, rectangle, img);
  }
  ...
  int draw(){
    ...
    //za ispisivanje bodova i zdravlja ako je potrebno
  ...
  Rectangle calculateBelow(){
     ...
     /* if(k2.getFallTime() == 1){
        ...
        */
        ...
        Rectangle temp = platforms.get(0);//r[0]
        //println(k2.isAbove(r[3]));
      //  println(" ");
        for(int i = 0; i < platforms.size(); i++){
          
          if(kirlia.isAbove(platforms.get(i))){//r[i]
            int distance = kirlia.howMuchAbove(platforms.get(i));//r[i]
           // println(distance + " " + (i + 1));
            if(distance < min){
              temp = platforms.get(i);//r[i]
              min = distance;
            }
          }
        }
  ...
  void setHealth(int healthK){
    kirlia.setHealth(healthK);
  }
  //Postavlja broj bodova od lika
  void setPoints(int pointsK){
    kirlia.setPoints(pointsK);
  }
  //Vraca broj zivota od lika
  int getHealt(){
    return kirlia.getHealth();
  }
  //Vraca broj bodova od lika
  int getPoints(){
    return kirlia.getPoints();
  }
